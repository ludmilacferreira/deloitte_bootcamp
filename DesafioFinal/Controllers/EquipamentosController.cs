...
namespace DesafioFinal.Controllers
{
    ...
    public class EquipamentosController : ControllerBase
    {
        ...
        // ============================================================
        // Metodos auxiliares de validacao e conversao de enums
        // ============================================================
        ...
        private static bool TryParseTipo(string valor, out TipoEquipamento tipo)
        {
            tipo = default;
            return valor switch
            {
                "Caminhao" => (tipo = TipoEquipamento.Caminhao) == tipo,
                "Escavadeira" => (tipo = TipoEquipamento.Escavadeira) == tipo,
                "Perfuratriz" => (tipo = TipoEquipamento.Perfuratriz) == tipo,
                "Carregadeira" => (tipo = TipoEquipamento.Carregadeira) == tipo,
                "Trator" => (tipo = TipoEquipamento.Trator) == tipo,
                _ => false
            };
        }

        private static bool TryParseStatus(string valor, out StatusOperacional status)
        {
            status = default;
            return valor switch
            {
                "Operacional" => (status = StatusOperacional.Operacional) == status,
                "EmManutencao" => (status = StatusOperacional.EmManutencao) == status,
                "Parado" => (status = StatusOperacional.Parado) == status,
                _ => false
            };
        }

        private static string TipoToString(TipoEquipamento tipo) => tipo switch
        {
        ...
        public async Task<IActionResult> Create([FromBody] CreateEquipamentoDto input)
        {
            ...
            // 2. Verificacao de unicidade do Codigo
            ...
            [FromQuery] string? codigo = null)
        {
            ...
            // Filtro por codigo (busca parcial, case-insensitive)
            ...
            var result = new PagedResultDto<EquipamentoResponseDto>
            {
                Page = page,
                PageSize = pageSize,
        ...
        public async Task<IActionResult> Update(int id, [FromBody] UpdateEquipamentoDto input)
        {
            ...
            // 3. Verificar duplicidade de codigo (se mudou)
        ...
        // (Somente se NAO houver manutencoes Concluidas associadas)
        // Como nao ha entidade Manutencao neste escopo minimo,
        // a regra fica preparada para extensao futura.
        // ============================================================
        [HttpDelete("{id:int}")]
        public async Task<IActionResult> Delete(int id)
        {
            ...
            // 2. Regra de negocio: impedir exclusao se houver manutencoes Concluidas
            // (Preparado para quando a entidade Manutencao for adicionada)
            // var temManutencaoConcluida = await _db.Manutencoes
            //     .AnyAsync(m => m.EquipamentoId == id && m.Status == StatusManutencao.Concluida);
            // if (temManutencaoConcluida)
            //     return Conflict(new { message = "Nao e possivel excluir: equipamento possui manutencoes concluidas." });
